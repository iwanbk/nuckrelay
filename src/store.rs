...
pub struct Store {
    peers_tx: RwLock<HashMap<String, Arc<Sender<Vec<u8>>>>>,
}
...
impl Store {
    ...
    pub fn add_peer(&self, peer_id: String, peer_tx: Arc<Sender<Vec<u8>>>) {
        let mut peers = self.peers_tx.write().unwrap();

        // If a peer with the same ID already exists, close it
        if let Some(old_peer) = peers.get(peer_id.as_str()) {
            //old_peer.close();
    ...
    pub fn delete_peer(&self, peer_id: &str, peer_tx: &Arc<Sender<Vec<u8>>>) {
        let mut peers = self.peers_tx.write().unwrap();
    ...
    pub fn get_peer(&self, id: &str) -> Option<Arc<Sender<Vec<u8>>>> {
        let peers = self.peers_tx.read().unwrap();
        peers.get(id).cloned()
    }

    /// Returns all the peers in the store
    pub fn get_all_peers(&self) -> Vec<Arc<Sender<Vec<u8>>>> {
        let peers = self.peers_tx.read().unwrap();
